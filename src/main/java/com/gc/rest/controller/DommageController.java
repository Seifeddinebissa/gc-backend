...
import com.gc.rest.dto.DommageDto;
...
public class DommageController {
	...
	@GetMapping("/dommage/intitule/{intitule}")
	public Object getByIntitule(@PathVariable String intitule) {
		Dommage dommage = dommageService.getDommageByIntitule(intitule);
		if (dommage != null) {
			DommageDto dommageDto = modelMapper.map(dommage, DommageDto.class);
			return ResponseEntity.status(HttpStatus.CREATED).body(dommageDto);
		}
